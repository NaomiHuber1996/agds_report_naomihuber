...
```{r}
#predictors: TA_F_MDS, SW_IN_F_MDS, LW_IN_F_MDS, VPD_F_MDS, CO2_F_MDS, PPFD_IN

#modify the data by 
#modified_data <- subset(daten, select = -c(siteid, TIMESTAMP, TA_F, SW_IN_F, LW_IN_F, VPD_F, PA_F, P_F, WS_F))
modified_data

best_rsquared <- 0 #Speicherung des besten R^2-Wertes
best_predictor <- "" #Speicherung des besten Prädiktors


for (predictor in names(modified_data)) {
  formel <- as.formula(paste("GPP_NT_VUT_REF ~", predictor))
...
cat("Bester Predictor: ", best_predictor, "\n")
cat("Bester R^2: ", best_rsquared, "\n")
cat("AIC: ", aic, "\n")

```

Regarding the output of the computation, we recognise the similarity with the calculated predictors in the previous part of the exercise. Therefore, we can confirm the solutions that the best predictor is "PPFD_IN" and the best R^2 is about 0.3627837. As well the lowest calculated AIC of this stepwise forward regression is the same as predicted in the previous part of the exercise which is about 45682.99 and corresponds to the AIC of model 4.

To get a more complex model of the data, one can add stepwise new predictors until the AIC will be higher than the previous one:
